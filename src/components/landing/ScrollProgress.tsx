...
const ScrollProgress = () => {
  ...
  // Transform progress to height - inverted (fills from top to bottom)
  ...
  return (
    <div className="fixed left-6 top-1/2 -translate-y-1/2 z-50 hidden lg:flex flex-col items-center gap-3">
      ...
      >
        ...
        {/* Celebration particles - only once */}
        {isComplete && (
          <>
            {[...Array(8)].map((_, i) => (
              <motion.div
      ...
      {/* Progress track with section dots - fills top to bottom */}
      <div className="relative w-2 h-80 bg-border/20 rounded-full overflow-visible">
        {/* Progress fill - now fills from top */}
        <motion.div
          className="absolute top-0 left-0 right-0 bg-gradient-to-b from-swap-green-light via-swap-green to-swap-green rounded-full"
          style={{ height: progressHeight }}
        />
        
        {/* Section dots - positioned from top */}
